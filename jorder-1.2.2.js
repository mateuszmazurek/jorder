...
jOrder.signature = function ($constants, $core) {
	...
	return function (fields, options) {
		// check presence
		if ((typeof fields === 'undefined' || !fields.length) && typeof fields !== "function") {
		...
		var self = {
			...
			signature: function (row, strict) {
				// returning signature
				if (!row) {
					return typeof fields === "function" ? $core.hash(fields.toString()) : escape(fields.join('_'));
				}
				// validating row
				var i, lookup;
				if (strict) {
			...
			keys: function (row) {
				...
				case $constants.text:
					...
					return (typeof fields === "function" ? fields.call(row) : row[fields[0]]).split(/\s+/g);
				default:
				case $constants.number:
...
jOrder.table = function ($core, $constants, $logging, $indexes, $selectors) {
	...
	return function (json, options) {
		...
		self = {
			...
			// logic execute functions for startof mode (implemented below)
			logics: {},
			...
			where: function (conditions, options) {
				...
				if (index) {
					...
					case $constants.startof:
					 ...
					 options.logic = options.logic || $constants.OR;
					 if(conditions.length > 0){
					 	var rowIds = [];
					 	for(var i = 0; i < conditions.length; i += 1){
					 		lower = $core.values(conditions[i])[0];
					 		rowIds.push(index.range({ lower: lower, upper: lower+'z' }, options));
					 	}
					 	rowIds = self.logics[options.logic](rowIds);
					 } else{
							rowIds = index.range({ lower: null, upper: null }, options);
						}
						break;
					default:
		...
		// execute AND-startof condition
		self.logics[$constants.AND] = function(rowIds){
			...
			while(rowIds.length > 1){
				_rowIds = [];
				for(i = 0; i < rowIds[0].length; i++){
					if(rowIds[1].indexOf(rowIds[0][i]) >= 0)
						_rowIds.push(rowIds[0][i]);
				}
				rowIds[0] = _rowIds;
				rowIds.splice(1, 1);
			}
		...
		// execute OR-startof condition
		self.logics[$constants.OR] = function(rowIds){
			while(rowIds.length > 1){
				for(i = 0; i < rowIds[1].length; i++){
					if(rowIds[0].indexOf(rowIds[1][i]) < 0)
						rowIds[0].push(rowIds[1][i]);
				}
				rowIds.splice(1,1);
			}
			return rowIds[0];
